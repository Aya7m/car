    ...
    <div className="relative min-h-screen md:flex flex-col items-center pt-8 max-w-13 md:max-w-60 border-r border-borderColor text-sm">
      <div className="group relative">
        <label htmlFor="image">
          <img
            src={
              image ? URL.createObjectURL(image) : user?.image || null // الرابط كامل من backend مباشرة
            }
            alt="image"
      ...
      {image && (
        ...
        >
          Save <img src={assets.check_icon} alt="icon" width={13} />
        </button>
      )}
        ...
        {ownerMenuLinks.map((link, index) => (
          ...
          >
            ...
            <div
              className={`${
                link.path === location.pathname &&
                "bg-primary w-1.5 h-8 rounded-lg right-0 absolute"
              }`}
            ></div>
          </NavLink>
        ))}
